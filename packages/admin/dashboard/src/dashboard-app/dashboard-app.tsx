    ...
    allMenuItems.forEach((item) => {
      ...
      if (item.path.includes("/:") || item.path.endsWith("/*")) {
        if (process.env.NODE_ENV === "development") {
          console.warn(
            `[@nedusa/dashboard] Menu item for path "${item.path}" can't be added to the sidebar as it contains a mandatory parameter.`
          )
        }
        return
      }
      ...
      if (isSettingsPath && pathParts.length > 2) {
        if (process.env.NODE_ENV === "development") {
          console.warn(
            `[@nedusa/dashboard] Nested settings menu item "${item.path}" can't be added to the sidebar. Only top-level settings items are allowed.`
          )
        }
      ...
      if (
        ...
        if (process.env.NODE_ENV === "development") {
          console.warn(
            `[@nedusa/dashboard] Nested menu item "${item.path}" can't be added to the sidebar as it is nested under "${parentItem.nested}".`
          )
        }
        return
      }
