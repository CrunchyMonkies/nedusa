  ...
  meta: {
    ...
    messages: {
      ifInWorkflowConstructor:
        "Do not use `if` statements inside a workflow constructor or a `when().then()` callback — both run at definition time, not execution time. Use `when(...).then(...)` from `@nedusa/framework/workflows-sdk` to conditionally execute steps.",
    },
    schema: [],
  },
  defaultOptions: [],
