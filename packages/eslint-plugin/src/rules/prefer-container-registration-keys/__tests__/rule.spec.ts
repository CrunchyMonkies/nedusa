...
ruleTester.run("prefer-container-registration-keys", rule, {
  ...
  invalid: [
    // resolve("query") with no existing import — adds the import.
    {
      ...
      output: `import { ContainerRegistrationKeys } from "@nedusa/framework/utils"\nconst query = req.scope.resolve(ContainerRegistrationKeys.QUERY)`,
    },
...
const logger = req.scope.resolve("logger")
      ...
      errors: [{ messageId: "preferRegistrationKey" }],
      output: `
import { Modules, ContainerRegistrationKeys } from "@nedusa/framework/utils"
...
const manager = req.scope.resolve(ContainerRegistrationKeys.MANAGER)
    ...
    {
      code: `
import { ContainerRegistrationKeys as CRK } from "@nedusa/framework/utils"
