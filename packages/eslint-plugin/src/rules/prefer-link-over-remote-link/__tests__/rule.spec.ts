...
ruleTester.run("prefer-link-over-remote-link", rule, {
  valid: [
    ...
    {
      code: `import type { Link } from "@nedusa/framework/types"`,
    },
  ...
  invalid: [
    ...
    // Type import: RemoteLink from @nedusa/framework/types.
    {
      code: `import type { RemoteLink } from "@nedusa/framework/types"`,
      errors: [{ messageId: "remoteLinkTypeImport" }],
      output: `import type { Link } from "@nedusa/framework/types"`,
    },
    // Type import: IRemoteLink from @nedusa/types.
    {
      code: `import { IRemoteLink } from "@nedusa/types"`,
      errors: [{ messageId: "remoteLinkTypeImport" }],
      output: `import { ILink } from "@nedusa/types"`,
    },
    // Mixed: rename only the deprecated specifier.
    {
      code: `import type { Link, RemoteLink } from "@nedusa/framework/types"`,
      errors: [{ messageId: "remoteLinkTypeImport" }],
      output: `import type { Link, Link } from "@nedusa/framework/types"`,
    },
    // Aliased import — rename only the `imported` part, keep alias.
    {
      code: `import type { RemoteLink as MyLink } from "@nedusa/framework/types"`,
      errors: [{ messageId: "remoteLinkTypeImport" }],
      output: `import type { Link as MyLink } from "@nedusa/framework/types"`,
    },
    // Multiple violations in one file.
    {
      code: `
        import { RemoteLink } from "@nedusa/framework/types"
      ...
      output: `
        import { Link } from "@nedusa/framework/types"
