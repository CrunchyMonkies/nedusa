...
/**
 * The `@mikro-orm/*` subpackages that `@nedusa/framework` re-exports as of
 * v2.11.0 (`@nedusa/framework/mikro-orm/<sub>`). Imports of these can be
...
/** `awilix` is re-exported wholesale as `@nedusa/framework/awilix`. */
const AWILIX_SOURCE = "awilix"
const AWILIX_REEXPORT = "@nedusa/framework/awilix"
...
function rewriteSource(value: string): string | null {
  ...
  if (value.startsWith(MIKRO_ORM_PREFIX)) {
    ...
    if (MIKRO_ORM_REEXPORTED_SUBPATHS.has(rest)) {
      return `@nedusa/framework/mikro-orm/${rest}`
    }
  }
...
export const rule = createRule<[], MessageIds>({
  ...
  meta: {
    ...
    docs: {
      description:
        "Don't import MikroORM or awilix directly. As of Medusa v2.11, they're bundled into `@nedusa/framework`; use the `@nedusa/framework/mikro-orm/*` and `@nedusa/framework/awilix` re-exports.",
    },
    messages: {
      useFrameworkReexport:
        "Import from `{{ replacement }}` instead of `{{ source }}`. MikroORM and awilix are re-exported by `@nedusa/framework` as of Medusa v2.11.",
      noDirectMikroOrmImport:
        "Don't import directly from `{{ source }}`. As of Medusa v2.11, MikroORM is bundled into `@nedusa/framework` — use a `@nedusa/framework/mikro-orm/*` re-export.",
    },
    fixable: "code",
    schema: [],
  },
