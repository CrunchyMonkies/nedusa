import { ModuleResolution } from "@nedusa/types"
import { createMedusaContainer } from "@nedusa/utils"
...
describe("modules loader", () => {
  ...
  it("should throw an error if the default package isn't found and the module is required", async () => {
    ...
    const moduleResolutions: Record<string, ModuleResolution> = {
      testService: {
        resolutionPath: "@nedusa/testService",
        definition: {
          key: "testService",
          defaultPackage: "@nedusa/testService",
          label: "TestService",
          isRequired: true,
          defaultModuleDeclaration: {
            scope: MODULE_SCOPE.INTERNAL,
    ...
    try {
      ...
      expect(err.message).toEqual(
        `Make sure you have installed the default package: @nedusa/testService`
      )
    }
  })
