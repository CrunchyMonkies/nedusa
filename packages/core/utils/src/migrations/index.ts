import { MikroORM, MikroORMOptions } from "@nedusa/deps/mikro-orm/core"
import {
  MigrateOptions,
  MigrationResult,
  UmzugMigration,
...
const replacements = [
  // MikroORM imports - replace mikro-orm/{subpath} with @nedusa/framework/mikro-orm/{subpath}
  {
    pattern: /from\s+['"]@?mikro-orm\/([^'"]+)['"]/g,
    // eslint-disable-next-line quotes
    replacement: 'from "@nedusa/framework/mikro-orm/$1"',
  },
  // PG imports - replace pg with @nedusa/framework/pg
  {
    pattern: /from\s+['"]pg['"]/g,
    // eslint-disable-next-line quotes
    replacement: 'from "@nedusa/framework/pg"',
  },
]
