...
describe("getResolvedPlugins | relative paths", () => {
  test("resolve configured plugins", async () => {
    await fs.createJson("node_modules/@nedusa/draft-order/package.json", {
      name: "@nedusa/draft-order",
      version: "1.0.0",
    })
    ...
    expect(plugins).toEqual(
      expect.arrayContaining([
        {
          id: "@nedusa/draft-order",
          modules: [],
          name: "@nedusa/draft-order",
          options: {},
          resolve: path.join(
            fs.basePath,
            "node_modules/@nedusa/draft-order/.medusa/server/src"
          ),
          version: "1.0.0",
        },
        {
