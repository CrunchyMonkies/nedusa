...
//
// Runs ONLY the `@nedusa/eslint-plugin` convention rules, using the exact
...
// An override whose every rule is a Medusa rule (`@nedusa/*`).
const isMedusaOverride = (o) =>
  o.rules &&
  Object.keys(o.rules).length > 0 &&
  Object.keys(o.rules).every((rule) => rule.startsWith("@nedusa/"))
